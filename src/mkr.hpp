...
class mkr{
private:
    static std::vector<uint8_t> LoadBytes(const char* path);
    static std::string LoadShaderFile(const char* path);
    static bool ShaderError(uint32_t& src);

    static std::vector<Verts> SetVertsVal(Vec2 position, Vec2 size, Color color);
    static std::vector<Verts> SetVertsVal(Vec3 position, Vec2 size, Color color);
